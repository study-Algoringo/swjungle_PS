...
for j in range(6):
    ...
    if j==0:
        ...
        if (lst[-1][0] == 3 and lst[j+1][0] == 3) or (lst[-1][0] == 4 and lst[j+1][0] == 4):
            minl = min(minl,lst[j][1])
    if j==5:
        if (lst[-1][0] == 1 and lst[0][0] == 1) or (lst[-1][0] == 2 and lst[0][0] == 2):
            # minw = min(minw,lst[j][1])
            minw = lst[j][1]
        if (lst[-1][0] == 3 and lst[0][0] == 3) or (lst[-1][0] == 4 and lst[0][0] == 4):
            # minl = min(minl,lst[j][1])
            minl = lst[j][1]


print((maxw*maxl-minw*minl)*n)
